...
class CraftMySonicPlaylistService:
    def __init__(
        self,
    ...
    async def create_cms_playlist(
        self, email: str, playlist_data: CreateCraftMySonicPlaylist
    ) -> SonicPlaylist:
        ...
        self.session.add(cms_playlist)
        await self.session.commit()
        return cms_playlist

    async def update_cms_playlist(
        self, email: str, playlist_id: UUID4, playlist_data: UpdateCraftMySonicPlaylist
    ) -> SonicPlaylist:
        ...
        playlist_record = await self.session.execute(
            ...
            .where(SonicPlaylist.playlist_type == PlaylistType.CRAFT_MY_SONIC)
        )
    ...
    async def get_all_cms_playlist_by_user(self, email: str):
        user_record = await self.session.execute(
            select(User).where(User.email == email)
        )
        user: User = user_record.scalar_one_or_none()
    ...
    async def get_cms_playlist_by_id(self, playlist_id: UUID4):
        playlist_record = await self.session.execute(
    ...
    async def get_user_cms_playlist_count(self, email: str):
        user_record = await self.session.execute(
            select(User).where(User.email == email)
        )
        user: User = user_record.scalar_one_or_none()
    ...
    async def get_featured_cms_playlist(self):
